...
public class BoidSimulation implements BoidSimulationBase {
  ...
  private Boid move(Boid actualBoid, List<Boid> boids) {
    Boid movedBoid =
        new Boid(actualBoid.getX(), actualBoid.getY(), actualBoid.getDx(), actualBoid.getDy());

    double vx1 =
        BoidTransformation.flyTowardsCenterDx(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getCohesionRange()),
            boidModel.getCohesionFactor());
    double vy1 =
        BoidTransformation.flyTowardsCenterDy(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getCohesionRange()),
            boidModel.getCohesionFactor());

    double vx2 =
        BoidTransformation.keepdistanceDx(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getSeparationRange()),
            boidModel.getSeparationFactor());
    double vy2 =
        BoidTransformation.keepDistanceDy(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getSeparationRange()),
            boidModel.getSeparationFactor());

    double vx3 =
        BoidTransformation.matchVelocityDx(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getAlignmentRange()),
            boidModel.getAlignmentFactor());
    double vy3 =
        BoidTransformation.matchVelocityDy(
            actualBoid,
            BoidTransformation.getNeighbors(actualBoid, boids, boidModel.getAlignmentRange()),
            boidModel.getAlignmentFactor());
    ...
    double speed =
        Math.sqrt(movedBoid.getDx() * movedBoid.getDx() + movedBoid.getDy() * movedBoid.getDy());
    if (speed > boidModel.getSpeedLimit()) {
      movedBoid.setDx(
          BoidTransformation.limitSpeedDx(actualBoid, speed, boidModel.getSpeedLimit()));
      movedBoid.setDy(
          BoidTransformation.limitSpeedDy(actualBoid, speed, boidModel.getSpeedLimit()));
    }
